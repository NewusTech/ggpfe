...
const onPage = async (event) => {    
    ...
    try {              
        const response = await fetch(`${apiBaseUrl}/api/master/adjustment?page=${event.page}&sortField=${event.sortField}&sortOrder=${event.sortOrder}&search=${search.value != null ? search.value : ''}&paginate_count=${event.rows}`, {
            method: 'GET',
            headers: {
...
onMounted(async () => {
    try {
        ...
        const response = await fetch(`${apiBaseUrl}/api/master/adjustment`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${localStorage.getItem('token')}`
...
const openNew = () => {
    ...
    modalHeader.value = "Add New Adjustment Category";
};
...
const updateDatamaster = async () => {
    try {
        const response = await fetch(`${apiBaseUrl}/api/master/adjustment/${datamaster.value.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${localStorage.getItem('token')}`
        ...
        if (response.ok) {
            onPage({ page: 0, search: search.value, sortField: dt.value?.sortField, sortOrder: dt.value?.sortOrder, rows: dt.value?.rows });            
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Data Adjs. Category Updated', life: 3000 });
        } else {
            throw new Error(data.error || 'Failed to update data Adjs. Category');
        }
    } catch (error) {        
...
const createDatamaster = async () => {
    try {
        ...
        if (response.ok) {
            ...
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Data Adjs. Category Created', life: 3000 });
        } else {
            throw new Error(data.error || 'Failed to create data Adjs. Category');
        }
    } catch (error) {        
        toast.add({ severity: 'error', summary: 'Failed', detail: error, life: 5000 });
    }
...
const editDatamaster = (editDatamaster) => {
    ...
    datamasterDialog.value = true;
    modalHeader.value = "Edit Adjustment Category";
};
...
const deleteDatamaster = async () => {
    try {
        ...
        if (response.ok) {
            ...
            datamaster.value = {};
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Adjustment Category Deleted', life: 3000 });
        } else {
            if (response.status === 500 && data.error.includes('Integrity constraint violation')) {                
                ...
                throw new Error(data.error || 'Failed to delete Adjustment Category');
            }
        }
    } catch (error) {        
            ...
            <div class="card">
                ...
                <Toolbar class="mb-4 p-0" style="border: none; background-color: white">
                    <template v-slot:start>
                        <div class="my-2">
                            <h5 class="m-0">Master Adjustment Category</h5>
                        </div>
                    </template>
                ...
                <Dialog v-model:visible="datamasterDialog" :style="{ width: '450px' }" :header="modalHeader" :modal="true" class="p-fluid">
                    <div class="field">
                        <label for="name">Adjustment Category</label>
                        <InputText id="name" v-model.trim="datamaster.name" required="true" autofocus :class="{ 'p-invalid': submitted && !datamaster.name }" />
                        <small class="p-invalid" v-if="submitted && !datamaster.name">Adjustment Category is required.</small>
                    </div>
