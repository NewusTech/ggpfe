...
const onPage = async (event) => {    
    ...
    try {              
        const response = await fetch(`${apiBaseUrl}/api/master/mvt?page=${event.page}&sortField=${event.sortField}&sortOrder=${event.sortOrder}&search=${search.value != null ? search.value : ''}&paginate_count=${event.rows}`, {
            method: 'GET',
            headers: {
...
onMounted(async () => {
    try {
        ...
        const response = await fetch(`${apiBaseUrl}/api/master/mvt`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${localStorage.getItem('token')}`
...
const openNew = () => {
    ...
    modalHeader.value = "Add New Movement Type";
};
...
const updateDatamaster = async () => {
    try {
        const response = await fetch(`${apiBaseUrl}/api/master/mvt/${datamaster.value.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${localStorage.getItem('token')}`
        ...
        if (response.ok) {
            ...
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Data Movement Type Updated', life: 3000 });
        } else {
            throw new Error(data.error || 'Failed to update data Movement Type');
        }
    } catch (error) {        
...
const createDatamaster = async () => {
    try {
        ...
        if (response.ok) {
            ...
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Data Movement Type Created', life: 3000 });
        } else {
            throw new Error(data.error || 'Failed to create data Movement Type');
        }
    } catch (error) {        
        toast.add({ severity: 'error', summary: 'Failed', detail: error, life: 5000 });
    }
...
const saveDatamaster = () => {
    ...
    if (datamaster.value.code && datamaster.value.code.trim() && datamaster.value.desc && datamaster.value.desc.trim()) {
...
const editDatamaster = (editDatamaster) => {
    ...
    datamasterDialog.value = true;
    modalHeader.value = "Edit Movement Type";
};
...
const deleteDatamaster = async () => {
    try {
        ...
        if (response.ok) {
            ...
            datamaster.value = {};
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Movement Type Deleted', life: 3000 });
        } else {
            if (response.status === 500 && data.error.includes('Integrity constraint violation')) {                
                ...
                throw new Error(data.error || 'Failed to delete Movement Type');
            }
        }
    } catch (error) {        
            ...
            <div class="card">
                ...
                <Toolbar class="mb-4 p-0" style="border: none; background-color: white">
                    <template v-slot:start>
                        <div class="my-2">
                            <h5 class="m-0">Master Movement Type</h5>
                        </div>
                    </template>
                ...
                >
                    ...
                    <Column field="code"  headerStyle="width:25%; min-width:10rem;">
                        <template #header>
                            <div class="flex flex-column md:flex-row md:justify-content-between md:align-items-center" style="text-align: right; width: 70%;">
                                <span style="width: 100%">MvT Code</span>
                            </div>
                        </template>
                    ...
                    <Column field="desc"  headerStyle="width:55%; min-width:10rem;">
                        <template #header>
                            <div class="flex flex-column md:flex-row md:justify-content-between md:align-items-center" :style="{ 'text-align': 'right', 'width': isSmallScreen ? '70%' : '58%' }">
                ...
                <Dialog v-model:visible="deleteDatamasterDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
                    <div class="flex align-items-center justify-content-center">
                        ...
                        <span v-if="datamaster"
                            >Are you sure you want to delete <b>{{ datamaster.code }}</b
                            >?</span
                        >
                    </div>
                    <template #footer>
