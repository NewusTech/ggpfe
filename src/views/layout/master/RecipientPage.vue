...
const onPage = async (event) => {    
    ...
    try {              
        const response = await fetch(`${apiBaseUrl}/api/master/good_recepient?page=${event.page}&sortField=${event.sortField}&sortOrder=${event.sortOrder}&search=${search.value != null ? search.value : ''}&paginate_count=${event.rows}`, {
            method: 'GET',
            headers: {
...
onMounted(async () => {
    try {
         ...
         const response = await fetch(`${apiBaseUrl}/api/master/good_recepient`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${localStorage.getItem('token')}`
...
const openNew = () => {
    ...
    modalHeader.value = 'Add New Good Recipient';
};
...
const updateDatamaster = async () => {
    try {
        const response = await fetch(`${apiBaseUrl}/api/master/good_recepient/${datamaster.value.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${localStorage.getItem('token')}`
        ...
        if (response.ok) {
            ...
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Data Good Recipient Updated', life: 3000 });
        } else {
            throw new Error(data.error || 'Failed to update data Good Recipient');
        }
    } catch (error) {        
...
const createDatamaster = async () => {
    try {
        ...
        if (response.ok) {
            ...
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Data Good Recipient Created', life: 3000 });
        } else {
            throw new Error(data.error || 'Failed to create data Good Recipient');
        }
    } catch (error) {        
        toast.add({ severity: 'error', summary: 'Failed', detail: error, life: 5000 });
    }
...
const saveDatamaster = () => {
    ...
    if (datamaster.value.code && datamaster.value.code.trim() && datamaster.value.desc && datamaster.value.desc.trim() && datamaster.value.department && datamaster.value.department.id !== null) {        
...
const editDatamaster = (editDatamaster) => {
    ...
    datamasterDialog.value = true;
    modalHeader.value = 'Edit Good Recipient';
};
...
const deleteDatamaster = async () => {    
    try {
        ...
        if (response.ok) {
            ...
            datamaster.value = {};
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Good Recipient Deleted', life: 3000 });
        } else {
            if (response.status === 500 && data.error.includes('Integrity constraint violation')) {                
                ...
                throw new Error(data.error || 'Failed to delete Good Recipient');
            }
        }
    } catch (error) {        
            ...
            <div class="card">
                ...
                <Toolbar class="mb-4 p-0" style="border: none; background-color: white">
                    <template v-slot:start>
                        <div class="my-2">
                            <h5 class="m-0">Master Good Recipient</h5>
                        </div>
                    </template>
                ...
                <DataTable ref="dt" :value="datamasters.data" dataKey="id" class="p-datatable-gridlines" :paginator="true"
                    ...
                    <Column field="code"  headerStyle="width:20%; min-width:10rem;">
                        <template #header>
                            <div class="flex flex-column md:flex-row md:justify-content-between md:align-items-center"
                    ...
                    <Column headerStyle="min-width:10rem;" style="text-align: center">
                        ...
                        <template #body="slotProps">
                            <Button v-if="hasEditMasterPermission" icon="pi pi-file-edit" class="p-button-text p-button-primary mt-2"
                                @click="editDatamaster(slotProps.data)" />
                            <Button v-if="hasDeleteMasterPermission" icon="pi pi-trash" class="p-button-text p-button-danger mt-2"
                                @click="confirmDeleteDatamaster(slotProps.data)" />
                        </template>
                    </Column>
                </DataTable>

                <Dialog v-model:visible="datamasterDialog" :style="{ width: '450px' }" :header="modalHeader" :modal="true"
                    class="p-fluid">
                    <div class="field">
                        <label for="code">Good Recipient</label>
                        <InputText id="code" v-model.trim="datamaster.code" required="true" autofocus
                            :class="{ 'p-invalid': submitted && !datamaster.code }" />
                        <small class="p-invalid" v-if="submitted && !datamaster.code">Good Recipient is required.</small>
                    </div>

                    <div class="field">
                        <label for="desc">Description</label>
                        <InputText id="desc" v-model.trim="datamaster.desc" required="true" autofocus
                            :class="{ 'p-invalid': submitted && !datamaster.desc }" />
                        <small class="p-invalid" v-if="submitted && !datamaster.desc">Desc is
                            required.</small>
                    </div>

                    <div class="field">
                        <label for="department" class="mb-3">Department</label>
                        <Dropdown id="department" v-model="datamaster.department" :options="departments"
                            optionLabel="name" placeholder="Select a Department"
                            :class="{ 'p-invalid': submitted && !datamaster.department }" required="true">
                ...
                <Dialog v-model:visible="deleteDatamasterDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
                    <div class="flex align-items-center justify-content-center">
                        ...
                        <span v-if="datamaster">Are you sure you want to delete <b>{{ datamaster.code }}</b>?</span>
                    </div>
                    <template #footer>
