...
const onPage = async (event) => {    
    ...
    try {              
        const response = await fetch(`${apiBaseUrl}/api/master/plant?page=${event.page}&sortField=${event.sortField}&sortOrder=${event.sortOrder}&search=${search.value != null ? search.value : ''}&paginate_count=${event.rows}`, {
            method: 'GET',
            headers: {
...
onMounted(async () => {
    try {      
        ...
        const response = await fetch(`${apiBaseUrl}/api/master/plant`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${localStorage.getItem('token')}`
...
const openNew = () => {
    ...
    modalHeader.value = "Add New Plant";
};
...
const updateDatamaster = async () => {
    try {
        const response = await fetch(`${apiBaseUrl}/api/master/plant/${datamaster.value.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${localStorage.getItem('token')}`
        ...
        if (response.ok) {
            ...
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Data Plant Updated', life: 3000 });
        } else {
            throw new Error(data.error || 'Failed to update data Plant');
        }
    } catch (error) {
...
const createDatamaster = async () => {
    try {
        ...
        if (response.ok) {
            ...
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Data Plant Created', life: 3000 });
        } else {
            // console.log(response)
            throw new Error(data.error || 'Failed to create data Plant');
        }
    } catch (error) {
        toast.add({ severity: 'error', summary: 'Failed', detail: error, life: 5000 });
    }
...
const deleteDatamaster = async () => {
    try {
        ...
        if (response.ok) {
            ...
            datamaster.value = {};
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Data Plant Deleted', life: 3000 });
        } else {
